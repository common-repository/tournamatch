 ...
 */
define( 'TOURNAMATCH_VERSION', '4.6.1' );
...
if ( ! function_exists( 'get_match_result_text' ) ) {
	...
	function get_match_result_text( $match ) {
		...
		if ( is_null( $one_name ) ) {
			...
			$one_name = sprintf( '<a href="%1$s">%2$s</a>', esc_url( trn_route( $route_name, array( 'id' => $match->one_competitor_id ) ) ), esc_html( $one_name ) );
		}
		if ( is_null( $two_name ) ) {
			$two_name = trn_get_option( 'tournament_undecided_display' );
		} else {
			$two_name = sprintf( '<a href="%1$s">%2$s</a>', esc_url( trn_route( $route_name, array( 'id' => $match->two_competitor_id ) ) ), esc_html( $two_name ) );
		}
...
if ( ! function_exists( 'trn_register_admin_scripts' ) ) {
	...
	function trn_register_admin_scripts() {
		wp_register_style( 'trn-admin-css', plugins_url( '/dist/css/admin.css', __FILE__ ), array(), '4.6.0' );
